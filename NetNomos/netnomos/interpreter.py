...
`ast.formula_to_string()` 只能把 AST 直接渲染成稳定字符串，
但它不知道运行时语义，例如：
- 某个数值 64 其实来自字段 `ip.ttl` 的 p50；
- 某个类别值来自 top1；
- 某个整数编码在 `enum_labels` 中对应 TCP。

本模块负责把原始 AST 渲染成更适合人读的文本表示，尤其会把：
1. 分位数常量渲染为 p50、p90；
2. 类别值渲染为 top1 等语义标签；
3. enum_labels 定义的整数/枚举渲染为可读名字。
...
def interpret_formula(
    ...
    `fields` 提供字段元数据，例如 enum_labels；
    `semantic_values` 提供运行时统计常量目录，例如 p50/top1。

    与 `formula_to_string()` 不同，这里会尽量把右侧常量解释成语义标签。
    """
    if isinstance(formula, BoolConst):
        return render_keyword("true") if formula.value else render_keyword("false")
    if isinstance(formula, Compare):
        # 比较公式是最常见的展示单元。
        # 左侧按 term 渲染，右侧优先走 interpret_constant()，尝试替换成 p50/top1/enum label。
        return (
...
def interpret_term(term: Term, fields: dict[str, FieldSpec]) -> str:
    ...
    这里主要处理结构，不处理右侧常量语义标签。
    常量语义标签需要知道它和哪个左侧字段比较，因此放在 `interpret_constant()` 中处理。
    """
...
def interpret_constant(
    ...
    """优先把右值解释成语义标签，否则退回原始值。

    解释顺序是：
    1. 查询运行时统计得到的 semantic_values；
    2. 查询字段自身的 enum_labels；
    3. 直接渲染原始字面量。
    """
    if not isinstance(right, Constant):
        # 右侧不是常量时，例如 field <= other_field，
        # 没有 p50/top1 这类常量语义可替换，直接按 term 渲染。
    ...
    if isinstance(left, SymbolRef) and left.name in fields:
        # 如果字段声明了 enum_labels，则把数字/字符串编码翻译成可读名称。
...
def render_value(value: Any) -> str:
    ...
    字符串不再额外包引号，便于规则阅读。
    例如输出 `protocol = TCP`，而不是 `protocol = 'TCP'`。
    """
