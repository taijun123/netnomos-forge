// NetNomos 原生 hitting-set 枚举器。
//
// 这个文件是 Python 版本搜索器的性能加速实现，核心目标是：
// 1. 用位图高效表示“某个谓词覆盖了哪些 evidence set”；
// 2. 用回溯搜索枚举极小 hitting sets；
// 3. 通过 pybind11 暴露给 Python 学习器调用。
...
bool is_subset(const std::vector<int>& lhs, const std::vector<int>& rhs) {
    // 输入向量默认有序，因此可以线性判断 lhs 是否为 rhs 的子集。
...
py::dict enumerate_hitting_sets(
    ...
    for (auto evidence : raw_evidence_sets) {
        // 每个 evidence set 先去重、排序，便于后续稳定搜索。
    ...
    branch = [&](const std::vector<int>& chosen, const DynamicBitset& covered, std::size_t covered_count) {
        ...
        std::sort(
            ...
            candidates.end(),
            [&](int lhs, int rhs) {
                // 优先尝试能够覆盖更多未覆盖 evidence 的谓词，提升剪枝效果。
